...
(function () {
  ...
  const STORAGE_KEY = "japan2026demo.order.v1";
  ...
  // ======================= ЧЕКЛИСТ ПОДГОТОВКИ =======================
  const CL_KEY = "japan2026demo.checklist.v2";
  ...
  const PACK_KEY = "japan2026demo.packing.ru-depart.v2";
  let packState = {};
  ...
  const FOLD_KEY = "japan2026demo.folds.v1";
  const GROUP_FOLD_KEY = "japan2026demo.groupFolds.v1";
  let foldState = {};
  let groupFoldState = {};
  ...
  function setupComfort() {
    ...
    if (!btn) return;
    let on = false;
    try { on = localStorage.getItem("japan2026demo.comfort") === "1"; } catch (e) {}
    const apply = () => {
    ...
    btn.addEventListener("click", () => {
      on = !on;
      try { localStorage.setItem("japan2026demo.comfort", on ? "1" : "0"); } catch (e) {}
      apply();
    });
  }
